...
cartRoutes.route('/addToCart').post(function (req, res) {
  console.log('Data Received', req.body.UserId,req.body.ProductId,req.body.quantity);
  ...
  try {
          ...
          CartModel.findOne({"UserId": {$eq:uid}}, async function(err, cart) {
            ...
            if (cart){
              ...
              CartModel.findOne({"UserId": {$eq:uid}, "items.ProductId": {$eq:pid}}, async function(err, product) {
                console.log('2nd Block');
                    if(product) {
                      ...
                      await query.exec();
                      console.log("done1");
                      res.status(200).json({'Cart': 'Product Updated Quantity successfully in Cart'});
                    }
                    else {
                      ...
                      await query2.exec();
                      console.log("done2");
                      res.status(200).json({'Cart': 'Product Updated (Created) successfully in Cart'}); 
                    } 
                  })
          } else {
            cart = new CartModel(
...
cartRoutes.route('/edit/:id/:pid').delete(function (req, res) {
      ...
      CartModel.findOne({"UserId": {$eq:userid}, "items.ProductId": {$eq:productID}}, async function(err, product) {
